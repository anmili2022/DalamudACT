...
Current metadata version: `0.15.2.13`
...
- ?? `Minimal / ????`???????? `Classic / Ikegami / Minimal` ???????
- ?????????????????????????/??/???????????????
- ????????????????????????????????
...
- ?? `tools/DotReconcile`?????? `history-records.json` ??? ACT `Network_*.log` ? DoT ?????
- ???? `hostile-only 24|DoT` ???????? `ACT ???` ? `??? hostile DoT`?
- `README.md` ??????????????????????
