...
- 设置窗口采用卡片式布局，常用项按功能分组展示
- 可以控制面板显示内容，也可以调整列宽、行高、透明度和颜色
- 可以直接设置 `锁定悬浮窗口`、共享列显示和 `历史记录预览时长（秒）`
...
- 如果觉得悬浮窗太挡视线：
  - 降低 `DPS统计面板透明度`
- 如果已经调好了统计页或历史页的列宽：
  - 直接保持当前宽度即可，插件会自动记忆到配置文件，下次打开时按上次宽度恢复
- 如果悬浮窗位置已经摆好：
  - 可以打开 `锁定悬浮窗口`，避免误拖动和误改大小
...
- `DPS / HPS / 承伤` 三个统计页共用同一组列显示开关和 `显示人数`
- 同一个开关在不同页中的含义如下：
  - `伤害列` = `DPS 伤害量 / HPS 治疗量 / 承伤 承伤量`
  - `秒伤列` = `DPS 秒伤 / HPS 秒疗 / 承伤 秒承伤`
- 伤害量使用中文单位显示，例如：
  - `9.68万`
  - `1.25亿`
  - `2.00兆`
...
- 这一组设置会同时作用于 `DPS / HPS / 承伤` 三个统计页
- `显示玩家列` / `显示职业列` / `显示死亡列` 会直接控制三个页签中的同名列
- `显示伤害列` 分别对应：
  - `DPS` 的 `伤害量`
  - `HPS` 的 `治疗量`
  - `承伤` 的 `承伤量`
- `显示秒伤列` 分别对应：
  ...
  - `HPS` 的 `秒疗`
  - `承伤` 的 `秒承伤`
- `显示人数` 会同时限制三个统计页显示多少条记录
- 如果你只想看前几名，可以把人数调小；如果想把整队都看出来，就把人数调大
- 切换这些列显示开关时，不会重置你当前已经拖好的列宽
- 统计页与历史页的列宽会自动记忆到配置文件，下次打开插件时会按上次宽度恢复
- 如果想清掉宽度记忆，可以使用：
  - `重置统计页列宽记忆`
  - `重置历史页列宽记忆`
...
- `主题色` 模式下可以统一调整 `主题色透明度`
- 这个透明度会同时作用于所有职业主题色
- 如果你只想微调“颜色深浅 / 观感厚度”，优先先调透明度，再决定是否改单职业 RGB

### 主题色调色板

- 可以统一调整主题色透明度
- 可以单独调整每个职业的占比条颜色（RGB）
- 支持一键 `恢复默认主题色`
...
- `恢复默认主题色` 会同时恢复主题色透明度和所有职业颜色
- 如果你之前手动改过职业主题色，而想回到这套默认治疗配色，可以直接点一次 `恢复默认主题色`
...
- `导出历史记录` 会把当前历史写到插件配置目录中的 `history-records.json`
- `导入历史记录` 会从同一个文件读取历史记录
- `清空历史` 会删除已经保存的历史战斗记录
- `历史记录预览时长（秒）` 用于控制你点击历史记录后，悬浮面板临时显示该快照的持续时间
- `恢复默认` 会把设置改回插件初始状态
...
### 单人解限 / 跟 NPC 队友进本的当前状态

最近这轮排查后，可以明确确认的部分是：

- 在部分单人解限 / 特殊副本场景里，玩家自己原本可能会出现“有进入战斗但完全没有数据”的情况；
- 当前最新代码已经修补了本地玩家身份获取逻辑；
- 截至收工前，**玩家自己的战斗流水与 DPS 统计已恢复正常**。

也就是说，如果你现在重新测试最新构建，至少玩家自己这条链路应该已经比之前稳定很多。

### 关于 NPC 队友 / 信赖 / 幻体

当前版本已经继续补强了这类对象的识别逻辑：

- 会尝试在战斗事件现场动态观察友方对象；
- 对于名字类似 `XX的幻体` 的对象，最新代码已经开始尝试按 `actorId + 名字` 直接纳入统计跟踪；
- 这部分逻辑主要是为了修复“日志里能看到幻体名字，但面板里没有单独一行”的问题。

但需要说明：

- 截至这轮收工前，**幻体是否已经在所有相关副本里稳定单独成行，仍然等待最后一轮用户现场复测确认**。
...
- 收紧哪些对象会参与“是否脱战”的判断；
- 避免无关的友方对象长时间保持 `InCombat`，导致整场战斗一直不结算。

当前结论是：

- 这部分修正已经写进代码；
- 但“是否已经彻底修复进入战斗后不结束战斗”的问题，仍然需要最后一轮现场确认。

### 新的战斗流水筛选方式

按当前版本，战斗流水里的角色筛选已经改成**下拉框**，不再是之前那种来回切换的形式。
